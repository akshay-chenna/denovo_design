...
hybrid() {
	for i in {1..487} # Number of serial runs on each core
		do
		line=`sed "${i}q;d" names.txt`
		rm -rf em${line}
		mkdir em$line
		cd em$line
...
EOF
		...
		# Make changes (From rosetta nomenclature) in the residue names so that charmm can recogize them. Also add the residue entires in merged.hdb (copy paste the L version and rename it to Deg take ALA entry and duplicate it and rename it to DALA)
...
gmx_mpi trjconv -f ${line}.pdb -s ${line}.pdb -o a.pdb << EOF
0
EOF
...
EOF
		...
		mpirun -np 1 -host `sed "$2q;d" $PBS_NODEFILE` gmx_mpi mdrun -v -s em.tpr -o em.trr -g em$line.log -c em$line.gro -ntomp 1
...
node=1
for task in {1..1} # Number of parallel runs
do
	node=$((node+1))
	hybrid $task $node &
done
